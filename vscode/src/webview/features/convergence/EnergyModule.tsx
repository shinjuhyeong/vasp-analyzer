...
export function EnergyModule(props: EnergyModuleProps): ReactElement {
  ...
  return <><Controls {...props} />{props.mode === "graph" ?
    ...
    <div className="detail-table-scroll"><table className="detail-table" aria-label="Energy terms"><thead><tr><th>Term</th><th>Kind</th><th>Value</th><th>Rank</th></tr></thead><tbody>
      {rows.map((row, index) => <tr key={`${row.key}:${index}`} data-rank={row.rank ?? undefined}><th scope="row">{row.rawLabel}</th><td>{row.kind}</td><td>{row.value} {row.unit}</td><td>{row.rank ? <span className={`detail-rank rank-${row.rank}`}>Rank {row.rank}</span> : "—"}</td></tr>)}
    </tbody></table></div>}</>;
}
