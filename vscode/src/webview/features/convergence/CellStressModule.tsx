...
const KB_TO_GPA = 0.1;
const gpa = (valueKb: number): string => String(Number((valueKb * KB_TO_GPA).toPrecision(15)));
...
export function CellStressModule(props: CellStressModuleProps): ReactElement {
  ...
  return <><header className="convergence-module-header"><h3>Cell &amp; Stress</h3>
    ...
    unavailable ? <p className="detail-unavailable" role="status">Cell and stress details unavailable for this ionic step.</p> : step && <div className="cell-stress-details"><p className="sign-convention">Pressure uses the VASP sign convention. 1 kB = 0.1 GPa.</p><dl>
      <dt>External pressure</dt><dd>{step.externalPressureKb === null ? "Unavailable" : <><span>{number(step.externalPressureKb)} kB</span> <span>{gpa(step.externalPressureKb)} GPa</span></>}</dd>
      <dt>Pulay stress</dt><dd>{step.pulayStressKb === null ? "Unavailable" : <><span>{number(step.pulayStressKb)} kB</span> <span>{gpa(step.pulayStressKb)} GPa</span></>}</dd>
