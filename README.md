...
Selective Dynamics uses `T` for movable and `F` for fixed components. Strongest-component and RMS metrics include only movable components. If constraints are unavailable (for example an OUTCAR-only calculation), constraint-derived force metrics are shown as unknown instead of assuming every component is free.
